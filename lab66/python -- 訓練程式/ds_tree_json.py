...
datacsv = pd.read_csv(str(pathlib.Path.cwd())+'\\Data.csv')
datacsv.loc[(datacsv['Contraceptive method used']==2) | (datacsv['Contraceptive method used']==3),'Contraceptive method used']=0
datacsv.loc[(datacsv['Husband\'s education']>=2),'Husband\'s education']=2
datacsv.loc[(datacsv['Wife\'s education']>=2),'Wife\'s education']=2
datacsv.loc[(datacsv['Standard-of-living index']>=2),'Standard-of-living index']=2
# datacsv=datacsv[datacsv['Wife\'s religion']==0]
# del datacsv['Wife\'s religion']
print(datacsv)
...
#資料內容
features = datacsv.columns[0:len(columns)-1]

#資料類別
label = datacsv.columns[len(columns)-1]

#資料索引
index = list(range(datacsv.shape[0]))

data = datacsv.to_numpy()
X = data[:,0:len(columns)-1]
y = data[:,len(columns)-1]
print(y)
